    ...
    def __do_bitcoin(self):
        ...
        for (block_height, block_hash) in new_blocks:
            ...
            for (i, unconfirmed_tx) in enumerate(self.unconfirmed_txs[::-1]):
                (block_timestamp, found_tx) = make_timestamp_from_block(
                    unconfirmed_tx.tip_timestamp.msg,
                    block,
                    block_height,
                    serde_txs=serde_txs)
            ...
            while sent_tx is None:
                ...
                try:
                    if self.btc_net == 'mainnet' or (self.btc_net != 'mainnet' and random.random() < self.btc_broadcast_ratio):
                        ...
                        logging.info("I am not broadcasting %s the tx to emulate mainnet low wee. Ratio is (%f)", (b2lx(signed_tx), self.btc_broadcast_ratio) )
